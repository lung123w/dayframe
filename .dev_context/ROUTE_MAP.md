...
Owner: `df-lead` · Last updated: 2026-09-20 (v1.1 — every verb, mount and period parameter re-checked against `src/App.jsx`, `src/components/Sidebar.jsx`, `server/index.js`, `server/routes/*.js` @ `master` `526f3b3`)
...
| `/api/financial-cards` | `GET /` · `GET /:id` · `POST /` · `PUT /:id` · `POST /:id/deactivate` · `POST /:id/reorder` | `GET /` **seeds 9 hardcoded cards when the table is empty** (`financialCards.js:31, 6-16`); `reorder` body `{direction:'up'|'down'}` swaps `displayOrder` with the neighbour; `deactivate` is a soft delete (`active=0`); no DELETE route |
